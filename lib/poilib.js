...
class PoiCont {
  ...
  add_geojson(pois) {
    // add geojson pois / pois: {geojson: [],targets: []}
    let children = [];
    console.log("PoiCont: addGeojson: Start.");
    pois.geojson.forEach((node, node_idx) => {
      // 既存Poiに追加
      let poi = {
        geojson: pois.geojson[node_idx],
        targets: pois.targets[node_idx],
      };
      poiCont.set_geojson(poi);
      if (poiCont.lnglats[node.id] == undefined) {
        // 初期登録時
        let ll = geoCont.flat2single(
    ...
    for (const child in children) {
      ...
      for (const polygon in poiCont.polygons) {
        if (child !== polygon) {
          // 同一要素はチェックしない
          if (turf.inside(pt1, poiCont.polygons[polygon])) {
            // ポリゴン内に存在する場合
  ...
  set_geojson(poi) {
    // add_geojsonのサブ機能
    let cidx = poiCont.pdata.geojson.findIndex(
      (val) => val.id == poi.geojson.id
    );
    if (cidx === -1) {
      // 無い時は追加
  ...
  } // osmidを元に親geojson全体を返す

  get_osmid(osmid) {
    // osmidを元にgeojsonと緯度経度、targetを返す
    let idx = poiCont.geoidx[osmid];
    return idx == undefined
      ? undefined
      : {
          geojson: poiCont.pdata.geojson[idx],
          lnglat: poiCont.lnglats[osmid],
          targets: poiCont.pdata.targets[idx],
        };
  }
  ...
  } // get activities by osmid

  getCatnames(tags) {
    // get Category Name & subname & tag
    if (poiCont.cat_cache[tags.id] !== undefined)
      return Array.from(poiCont.cat_cache[tags.id]); // 配列をコピーして返す(参照返しだと値が壊れる)
    let catname = "",
      mainkey = "",
      mainval = "";
    let mainkeys = Conf.category_keys.filter(
      (key) => tags[key] !== undefined && key !== "*"
    ); // srarch tags
    ...
    if (subtag !== undefined) {
      for (let subkey of Object.keys(subtag)) {
        // subkey: ex: religion
        if (subcatname !== "") break;
        for (let subval of Object.keys(subtag[subkey])) {
          // subval: ex: shinto
  ...
  getTarget(target) {
    ...
    switch (target) {
      ...
      case "activity":
        ...
        break;
      default:
        acts = poiCont.adata.filter((a) => {
          return a.category == target;
        });
        break;
    }
  ...
  getPois(targets) {
    ...
    let geojson = poiCont.pdata.geojson.filter((geojson_val, geojson_idx) => {
      ...
      for (let target_idx in poiCont.pdata.targets[geojson_idx]) {
        if (
          targets.includes(poiCont.pdata.targets[geojson_idx][target_idx]) ||
          targets == "-"
        ) {
  ...
  makeList(targets) {
    ...
    let pois = poiCont.getPois(targets),
      datas = [],
      listed = {}; // targetsに指定されたpoiのみフィルター
      ...
      if (geoCont.checkInner(pois.lnglat[idx], LL)) {
        ...
        Conf.list.columns.poi_fields.forEach((key) => {
          if (key == "#category") {
            ...
            let ptag,
              pGeo = poiCont.get_parent(tags.id);
    ...
    if (targets.indexOf(Conf.google.targetName) > -1) {
      // targetsにgSheet名があればリストに追加
        ...
        if (line !== undefined) {
          ...
          switch (newmode) {
            ...
            default:
              ...
              if (poi !== undefined) {
                let allActs = Conf.listTable.allActs
                  ? true
                  : geoCont.checkInner(poi.lnglat, LL);
                if (allActs && listed[poi.geojson.properties.id] !== true) {
                  ...
                  Conf.list.columns.act_fields.forEach((key) => {
                    if (key.indexOf("datetime") > -1) {
                      ...
                      let osmtag =
                        poi !== undefined
                          ? poi.geojson.properties[tagname]
                          : ""; // OSM tag名を指定した場合
                      data.push(osmtag);
                    } else {
...
class PoiMarker {
  ...
  setPoi(target, actonly, flist) {
    const checkMarker = function (poi, LL, zoomlv) {
      ...
      if (geoCont.checkInner(poi.lnglat, LL)) {
        ...
        let tg2 = tg1.filter((item) =>
          Object.keys(Conf.view.poiZoom).includes(item)
        ); // 重複targetsを取得
        let tgv = false;
        for (let tg3 of tg2) {
          if (zoomlv >= Conf.view.poiZoom[tg3]) tgv = true;
        } // ズーム範囲内がチェック
        if (viewflag && !poiMarker.#osmpois.includes(poi.geojson.id) && tgv) {
          // act ok and 未表示の場合
          params = {
            id: poi.geojson.id,
            target: target,
    ...
    const makeMarker = function (params) {
      // Make Marker(Sometimes multiple markers are returned)
      return new Promise((resolve, reject) => {
        let tags =
          params.poi.geojson.properties.tags == null
            ? params.poi.geojson.properties
            : params.poi.geojson.properties.tags;
        let name =
          tags[params.langname] == null ? tags.name : tags[params.langname];
        ...
        params.span_width =
          name !== "" ? name.length * Conf.effect.text.size : 0;
        let css_name = actlists.length > 0 ? "attention" : "normal";
        params.size = Conf.icon[css_name];
        params.html += `<img class="${css_name}" src="./${
          Conf.icon.path
        }/${poiMarker.get_icon(
          tags
        )}" icon-name="${name}" onclick="cMapMaker.viewDetail('${
          params.poi.geojson.id
        }')">`;
    ...
    const saveMarker = function (params) {
      if (poiMarker.#osmpois.indexOf(params.poi.geojson.id) == -1) {
        ...
        makeMarker(params).then((marker) => {
          if (marker !== null)
            marker.forEach((val) => poiMarker.#markers[target].push(val)); // 複数Marker対応
        });
      }
    };
    ...
    if (flist !== undefined) {
      ...
      flist.forEach((list) => {
        let inActs = Object.keys(Conf.activities).indexOf(
          list[0].split("/")[0]
        );
      ...
      // set target(activityはマーカーを手間表示で後回し)
      let org_target = target,
        poisOrder = [];
      target = target == "-" ? "" : target;
      if (target !== "")
        target =
          Object.keys(Conf.view.poiZoom).indexOf(target) > -1
            ? target
            : "activity";
      ...
      if (all.pois.geojson !== undefined) {
        // pois表示
        all.pois.geojson.forEach(function (geojson, idx) {
          let poi = {
            geojson: geojson,
            targets: all.pois.targets[idx],
            lnglat: all.pois.lnglat[idx],
          };
          let params = checkMarker(poi, LL, zoomlv);
          if (Object.keys(params).length > 0 && params.actlists.length == 0) {
            // check ok(params内容あり)
      ...
      if (all.acts.length > 0) {
        ...
        all.acts.forEach((act) => {
          if (!poiMarker.#osmpois.includes(act.osmid)) {
            ...
            if (
              osm !== undefined &&
              (act.category == org_target || org_target == "activity")
            ) {
              let poi = {
                geojson: osm.geojson,
                targets: osm.targets,
  ...
  get_osmid(osmid) {
    ...
    for (let target of Object.keys(Conf.view.poiZoom)) {
      if (poiMarker.#markers[target] !== undefined) {
        let idx = poiMarker.#markers[target].findIndex(
          (val) => val.mapmaker_id == osmid
        );
        if (idx > -1) marker = poiMarker.#markers[target][idx];
        break;
      }
  ...
  delete_all() {
    Object.keys(Conf.view.poiZoom).forEach((target) =>
      poiMarker.delete(target)
    );
  ...
  delete(target, osmid) {
    // Marker delete * don't set pdata
    if (osmid == null || osmid == "") {
      // all osmid
      if (poiMarker.#markers[target] !== undefined) {
        poiMarker.#markers[target].forEach((marker) => {
          poiMarker.#osmpois = poiMarker.#osmpois.filter(
            (n) => n !== marker.mapmaker_id
          );
      ...
      poiMarker.#osmpois = poiMarker.#osmpois.filter(
        (n) => n !== poiMarker.#markers[target][idx].mapmaker_id
      );
